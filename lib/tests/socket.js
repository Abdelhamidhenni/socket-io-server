	...
	describe('Client connected.', function (done) {
		before(function (done) {
			this.client.on('connection', function (data) {
				...
				// Expose the web socket client id.
		...
		it('Event data object contains "wsClientId" property.', function () {
			expect(this.eventData).to.have.property('wsClientId');
		});
